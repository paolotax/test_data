class StacksLayout < UICollectionViewLayout
  ...
  def layoutAttributesForItemAtIndexPath(path)
    SpringboardLayoutAttributes.layoutAttributesForCellWithIndexPath(path).tap do |attrs|

      if self.isDeletionModeOn
        attrs.deleteButtonHidden = false
      else
        attrs.deleteButtonHidden = true
      end
  ...
  def layoutAttributesForSupplementaryViewOfKind(kind, atIndexPath:path)
    return nil 
    # unless kind == SmallConferenceHeader.kind
  ...
  def pinchedStackIndex=(new_index)  
    ...
    unless self.pinching == was_pinching
      @grid_layout = @pinching ?  SpringboardLayout.alloc.init : nil                    
    end
    self.invalidateLayout
  end
